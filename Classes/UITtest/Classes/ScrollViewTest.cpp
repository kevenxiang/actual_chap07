...
void ScrollViewTest::onEnter() {
    ...
    // 设置滚动层容器的位置坐标
    scrollView->setPosition(Vec2((visibleSize.width - backgroundSize.width)/2.0f + (backgroundSize.width - scrollView->getContentSize().width)/2.0f, (visibleSize.height - backgroundSize.height) / 2.0f + (backgroundSize.height - scrollView->getContentSize().height) / 2.0f));
    // 将滚动层容器添加为当前层的子节点
    ...
    button->setPosition(Vec2(button->getContentSize().width/2.0f, scrollView->getInnerContainerSize().height - button->getContentSize().height / 2.0f));
    // 将Button添加为当前层的子节点
    ...
    imageView->setPosition(Vec2(innerWidth - button_scale9->getPosition().x, button_scale9->getBottomBoundary() - button_scale9->getContentSize().height / 2.0f));
